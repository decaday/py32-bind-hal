...
pub enum HalError{
    UnknownPortError,
    PinNumOutOfRangeError,
    PortNumOutOfRangeError,
}
...
impl AnyPin {
    ...
    pub fn new(port_char:char, pin_num:u8) -> Result<Self, HalError> {
        let port_num = match port_char{
            #[cfg(feature = "peri_gpioa")]
            "A" | "a" => Ok(0),
            #[cfg(feature = "peri_gpiob")]
            "B" | "b" => Ok(1),
            #[cfg(feature = "peri_gpiof")]
            "F" | "f" => Ok(5),
            _ => HalError::UnknownPortError,
        }?;

        if pin_num > 15 {
            return HalError::PinNumOutOfRangeError
        }
        let pin_port = port_num * 16 + pin_num;
        Self { pin_port }
    }

    pub fn new_num(port_num:u8, pin_num:u8) -> Self {
        let port_num = match port_num{
            #[cfg(feature = "peri_gpioa")]
            0 => Ok(0),
            #[cfg(feature = "peri_gpiob")]
            1 => Ok(1),
            #[cfg(feature = "peri_gpiof")]
            5 => Ok(5),
            _ => HalError::PortNumOutOfRangeError,
        }?;

        if pin > 15 {
            return HalError::PinNumOutOfRangeError
        }
        let pin_port = port * 16 + pin;
        Self { pin_port }
    }
